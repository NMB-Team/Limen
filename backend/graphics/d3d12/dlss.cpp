...
HL_PRIM int HL_NAME(init)(bool showConsole) {
    ...
    pref.numFeaturesToLoad = sizeof(featureList) / sizeof(*featureList);
...
HL_PRIM DLSSOptimalSettings* HL_NAME(get_optimal_settings)(DLSSOptions* options) {
    ...
    DLSSOptimalSettings* outOptimalSettings = (DLSSOptimalSettings*)hl_gc_alloc_noptr(sizeof(DLSSOptimalSettings));
...
HL_PRIM int HL_NAME(evaluate_feature)(sl::FrameToken* frameToken, ID3D12GraphicsCommandList* cmdList, DLSSFeature feature) {
    ...
    sl::Result result = slEvaluateFeature(toSlFeature(feature), *frameToken, inputs, sizeof(inputs) / sizeof(*inputs), cmdList);
    return static_cast<int>(result);
}
